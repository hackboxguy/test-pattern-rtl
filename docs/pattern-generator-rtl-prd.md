...
| **Status** | Design spec. **Mode A is implemented & hardware-validated** — for current as-built status see [`CLAUDE.md`](../CLAUDE.md) and [`boards/tangnano9k/README.md`](../boards/tangnano9k/README.md). Implementation evolved some details — the §8.3 pattern table and §10.1 `PATSEL_W=4` are superseded: as-built is **32 patterns** (incl. 2D + 1D local-dimming), `PATSEL_W=5`, Tang Nano clean to 1024×768 (720p ELVDS-limited). |
